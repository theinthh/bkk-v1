...
def main(con):
    ...
    sql = f"SELECT hour, on_time_count, late_count FROM entry_lists WHERE list_id > {start_of_day};"
    ...
    #fill in the rest with average data from previous same weekdays
    ...
    sql_pred = f"SELECT hour, on_time_count, late_count FROM entry_lists WHERE weekday = {day} AND hour > {hour};"
